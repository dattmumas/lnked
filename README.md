...
Custom nodes (e.g., ImageNode, PollNode) defined in components/editor/nodes/_ to support rich content embedding.
...
github.com
github.com
. The middleware runs on Edge for every request to matching routes (/dashboard/_, /sign-in, /sign-up)
github.com
:
...
github.com
.
If GPT changes route structures or adds new protected sections, update the middleware.ts matcher and logic accordingly. For example, if adding an admin section, you might extend the matcher to protect /admin/_ as well.
...
github.com
) lists all necessary env vars. GPT should not introduce new env vars without reason. If absolutely needed, document them and update the example file. Use the NEXT_PUBLIC_ prefix for any var that needs to be exposed to client-side (and only those). For instance, we use NEXT_PUBLIC_SUPABASE_URL for Supabase because the browser needs it
github.com
...
## Changelog Enforcement (Critical Rule)

- **Every code change (in src/ or app/ or any code file) MUST be accompanied by an update to `change_log.json` at the project root.**
- The changelog entry must include a UUID, the file path, and a brief summary of the change (see below for format).
- The CI pipeline will block any PR that changes code but does not update `change_log.json`.
- This rule is strictly enforced for all contributors (human or AI).
