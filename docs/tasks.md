...
2. Front-End Scaffolding and Page Organization
   ...
   GitHub
   GitHub
   . Both cards share an info footer with author name, collective, timestamp, and stat buttons (like, comment, etc.), which can be abstracted as a sub-component (e.g. PostCardFooter). This improves maintainability: adding a new stat (say, “Shares”) or changing styling can be done in one place. It also ensures visual consistency across feed, profile, and collective pages – all use the same Card components but with appropriate data.
