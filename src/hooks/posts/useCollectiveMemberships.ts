...
export const useCollectiveMembershipCounts = (collectiveIds: string[]) => {
  return useQuery({
...
export const useCollectivePostingPermissions = (collectiveIds: string[]) => {
  const { user } = useUser();

  return useQuery({
    queryKey: ['collective-posting-permissions', user?.id, collectiveIds],
    queryFn: async (): Promise<Record<string, boolean>> => {
      if (!user?.id || collectiveIds.length === 0) {
        return {};
      }
...
export const useSearchCollectiveMemberships = (searchQuery: string) => {
  const { user } = useUser();
