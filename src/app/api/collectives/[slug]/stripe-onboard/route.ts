...
// Cache configuration
const CACHE_CLEANUP_INTERVAL =
  MINUTES_PER_HOUR * SECONDS_PER_MINUTE * MILLISECONDS_PER_SECOND; // 1 hour
const CACHE_TTL =
  HOURS_PER_DAY *
  MINUTES_PER_HOUR *
  SECONDS_PER_MINUTE *
  MILLISECONDS_PER_SECOND; // 24 hours
...
const accountLinkCache = new Map<
  string,
  {
    url: string;
    createdAt: number;
...
export async function POST(
  ...
  const logger = createAPILogger(req, '/api/collectives/[slug]/stripe-onboard');
  ...
  try {
    ...
    if (
      ...
      recordAPIMetrics({
        endpoint: '/api/collectives/[slug]/stripe-onboard',
        method: 'POST',
    ...
    if (
      ...
      const { accountId, isNew } = await getOrCreateStripeAccount(
        stripe,
        collectiveId,
      );
