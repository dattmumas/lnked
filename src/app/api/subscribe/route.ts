...
const RATE_LIMIT_MAX_REQUESTS = parseInt(
  process.env['SUBSCRIBE_RATE_LIMIT_MAX'] ?? DEFAULT_RATE_LIMIT_MAX.toString(),
  10,
);
const RATE_LIMIT_WINDOW_MS =
  MINUTES_PER_HOUR * SECONDS_PER_MINUTE * MS_PER_SECOND;
const IDEMPOTENCY_WINDOW_MINUTES = parseInt(
  process.env['SUBSCRIBE_IDEMPOTENCY_WINDOW_MINUTES'] ??
    DEFAULT_IDEMPOTENCY_WINDOW_MINUTES.toString(),
  10,
);
const IDEMPOTENCY_WINDOW_MS =
  IDEMPOTENCY_WINDOW_MINUTES * SECONDS_PER_MINUTE * MS_PER_SECOND;
const PRICE_CACHE_MINUTES = parseInt(
  process.env['SUBSCRIBE_PRICE_CACHE_MINUTES'] ??
    DEFAULT_PRICE_CACHE_MINUTES.toString(),
  10,
);
...
const getAllowedPaymentMethods = ():
  ...
  if (
    envMethods !== null &&
    envMethods !== undefined &&
    envMethods.length > 0
  ) {
    const methods = envMethods
      .split(',')
      .map((m) => m.trim())
      .filter((m) => SUPPORTED_PAYMENT_METHODS.has(m as never));
    return methods.length > 0
      ? (methods as Stripe.Checkout.SessionCreateParams.PaymentMethodType[])
      : undefined;
  }
...
const SubscribeRequestSchema = z.object({
  priceId: z
    ...
    .max(MAX_PRICE_ID_LENGTH, 'Price ID too long'),
  redirectPath: z
    .string()
    .max(
      MAX_REDIRECT_PATH_LENGTH,
      `Redirect path must be ${MAX_REDIRECT_PATH_LENGTH} characters or less`,
    )
    .regex(
      /^\/[^/]/,
      'Redirect path must start with / and not be protocol-relative',
    )
    .optional()
    .default('/'),
  targetEntityType: z.enum(['user', 'collective'], {
    errorMap: () => ({
      message: 'Target entity type must be either "user" or "collective"',
    }),
  }),
  ...
  const sanitized = {
    ...
    email:
      typeof context['email'] === 'string'
        ? `***@${context['email'].split('@')[1] || 'unknown'}`
        : undefined,
    // Remove any other sensitive fields
    stripCustomerId:
      typeof context['stripCustomerId'] === 'string' ? 'cus_***' : undefined,
  };
...
async function validatePriceId(
  ...
  try {
    ...
    const allowedPrices =
      allowedPricesEnv !== null && allowedPricesEnv !== undefined
        ? allowedPricesEnv.split(',').map((p) => p.trim())
        : [];
    ...
    logSubscriptionAction(
      { action: 'price_validation_failed', priceId, error },
      'error',
    );
...
async function checkRateLimit(
  ...
  try {
    ...
    if (error !== null) {
      logSubscriptionAction(
        { action: 'rate_limit_check_failed', userId, error: error.message },
        'warn',
      );
    ...
    logSubscriptionAction(
      { action: 'rate_limit_error', userId, error },
      'warn',
    );
...
async function checkIdempotency(
  ...
  try {
    ...
    if (error !== null) {
      logSubscriptionAction(
        { action: 'idempotency_check_failed', userId, error: error.message },
        'warn',
      );
    ...
    logSubscriptionAction(
      { action: 'idempotency_error', userId, error },
      'warn',
    );
    return { isDuplicate: false };
  }
...
async function storeIdempotencyRecord(
  ...
  try {
    ...
    logSubscriptionAction(
      { action: 'idempotency_store_failed', userId, error },
      'warn',
    );
  }
}
...
async function checkCollectiveAuthorization(
  ...
  try {
    ...
    if (error !== null) {
      ...
      return {
        authorized: false,
        error: 'Failed to verify collective permissions',
      };
    }
...
async function getOrCreateStripeCustomer(
  user: {
    id: string;
    email: string;
    user_metadata?: { full_name?: string; preferred_locales?: string[] };
  },
  supabase: SBClient,
  ...
  try {
    ...
    // Stripe client is now guaranteed non-null
    const validatedStripe = stripe as NonNullable<typeof stripe>;
    ...
    // If we already have a Stripe customer ID, verify it actually exists (can be deleted in dashboard)
    const maybeExistingId = existingCustomer?.stripe_customer_id ?? null;
    if (maybeExistingId && maybeExistingId.length > 0) {
      try {
        await validatedStripe.customers.retrieve(maybeExistingId);
        // Retrieval succeeded → ID is valid
        return { success: true, customerId: maybeExistingId };
      } catch (err: unknown) {
        if (
          err !== null &&
          typeof err === 'object' &&
          ...
          logSubscriptionAction(
            {
              action: 'stale_customer_id_detected',
              userId: user.id,
              staleCustomerId: maybeExistingId,
            },
            'warn',
          );
        } else {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  // Helper – ensure the Stripe Price exists in the *platform* account
  async function ensurePlatformPrice(
    priceId: string,
    connectedAccountId: string,
    planName: string,
  ): Promise<string> {
    try {
      // Attempt to retrieve in platform account first
      await validatedStripe.prices.retrieve(priceId);
      return priceId; // already good
    } catch (err: unknown) {
      if (
        err !== null &&
        typeof err === 'object' &&
        ...
        // Fetch price from connected account, then clone
        const connectedStripePrice = (await validatedStripe.prices.retrieve(
          priceId,
          { stripeAccount: connectedAccountId },
        )) as Stripe.Price;

        // Ensure product in platform account
        let platformProductId: string;
        if (
          typeof connectedStripePrice.product === 'string' &&
          connectedStripePrice.product.length > 0
        ) {
          // minimal clone – new product with same name
          const prod = await validatedStripe.products.create({
            name: planName,
            metadata: { cloned_from_product: connectedStripePrice.product },
          });
          platformProductId = prod.id;
        } else {
          const prod = await validatedStripe.products.create({
            name: planName,
          });
        ...
        const recurringParam = connectedStripePrice.recurring
          ? {
              interval: connectedStripePrice.recurring.interval as
                | 'day'
                | 'week'
                | 'month'
                | 'year',
              ...(connectedStripePrice.recurring.interval_count
                ? {
                    interval_count:
                      connectedStripePrice.recurring.interval_count,
                  }
        ...
        const newPrice = await validatedStripe.prices.create({
          product: platformProductId,
          currency: connectedStripePrice.currency,
          unit_amount: connectedStripePrice.unit_amount!,
          ...(recurringParam ? { recurring: recurringParam } : {}),
          metadata: { cloned_from_price: priceId },
        });
  ...
  try {
    ...
    if (
      ...
      idempotencyResult.existingUrl !== null &&
      idempotencyResult.existingUrl !== undefined
    ) {
      logSubscriptionAction(
        {
        ...
        'warn',
      ); // This is info level - successful dedup
      return NextResponse.json({ url: idempotencyResult.existingUrl });
    }

    // 2. Lookup subscription plan to determine owner + connected account
    const { data: planRow, error: planErr } = await supabase
      .from('subscription_plans')
      .select('owner_type, owner_id, price_snapshot, stripe_price_id')
      .eq('stripe_price_id', priceId)
      .eq('active', true)
      .maybeSingle();

    if (planErr !== null || planRow === null) {
      return NextResponse.json(
    ...
    if (planRow.owner_type === 'user') {
      const { data: owner, error: ownerErr } = await supabase
        .from('users')
        .select(
          'stripe_account_id, stripe_charges_enabled, stripe_payouts_enabled',
        )
        .eq('id', planRow.owner_id)
        .maybeSingle();

      const accountId = (owner as { stripe_account_id: string | null } | null)
        ?.stripe_account_id;

      if (ownerErr !== null || accountId === null || accountId === undefined) {
        return NextResponse.json(
          { error: 'Creator is not ready to accept payments' },
          { status: BAD_REQUEST_STATUS },
        );
      }
      ...
      return NextResponse.json(
        { error: 'Collective subscriptions not yet supported' },
        { status: BAD_REQUEST_STATUS },
      );
    }
    ...
    // Derive human-readable plan name for product cloning
    let derivedPlanName = 'Subscription Plan';
    if (
      planRow.price_snapshot &&
      typeof planRow.price_snapshot === 'object' &&
      (planRow.price_snapshot as Record<string, unknown>)['product'] !==
        undefined
    ) {
      const prod = (planRow.price_snapshot as Record<string, unknown>)[
        'product'
      ] as { name?: string } | string;
      if (typeof prod === 'object' && prod && 'name' in prod) {
        derivedPlanName = (prod as { name?: string }).name ?? derivedPlanName;
      }
    }

    const platformPriceId = await ensurePlatformPrice(
      priceId,
      connectedAccountId,
      derivedPlanName,
    );
    ...
    if (targetEntityType === 'collective') {
      const authResult = await checkCollectiveAuthorization(
        user.id,
        targetEntityId,
        supabase,
      );
    ...
    // Get or create platform-level customer (has saved cards)
    ...
    const defaultPaymentMethod = await fetchDefaultPaymentMethod(
      customerResult.customerId,
    );
    const hasDefaultPM = defaultPaymentMethod !== null;
    ...
    const transferData: { destination: string } | undefined = undefined;
    // skip collective payment logic for now

    // Generate Stripe idempotency key for checkout session
    const idempotencyKey = `checkout_${user.id}_${targetEntityType}_${targetEntityId}_${platformPriceId}`;
    ...
    if (!hasDefaultPM) {
      ...
      if (
        ...
        checkoutSession.url === undefined ||
        checkoutSession.url.length === 0
      ) {
        logSubscriptionAction(
          {
      ...
      // Store session & idempotency record (same as before)
    ...
    // ---------- HAS DEFAULT CARD → create subscription directly ----------
    const subIdemKey = `subscribe_${user.id}_${platformPriceId}`;

    const subscription = await validatedStripe.subscriptions.create(
      {
        customer: customerResult.customerId,
    ...
    logSubscriptionAction(
      {
        action: 'subscription_created_direct',
        userId: user.id,
