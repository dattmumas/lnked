...
const NOTIFICATION_PAGE_LIMIT_MAX = parseInt(
  process.env['NOTIF_PAGE_LIMIT_MAX'] || '100',
  10,
);
const NOTIFICATION_PAGE_OFFSET_MAX = parseInt(
  process.env['NOTIF_PAGE_OFFSET_MAX'] || '10000',
  10,
);
const NOTIFICATION_MAX_IDS_PER_REQUEST = parseInt(
  process.env['NOTIF_MAX_IDS_PER_REQUEST'] || '50',
  10,
);
...
export const notificationActionSchema = z.object({
  notification_ids: z
    ...
    .max(
      ...
      `Maximum ${NOTIFICATION_MAX_IDS_PER_REQUEST} notification IDs allowed per request`,
    )
    .refine(
